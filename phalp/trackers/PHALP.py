...
class PHALP(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # pre-process
        self.preprocess_data = dict()
        self.to(self.device)
        self.preprocessed_data_file_name = self.cfg.video.source[:-4] + '_preprocessed_data.pkl'
        if os.path.isfile(self.preprocessed_data_file_name):
            _fp = open(self.preprocessed_data_file_name, "rb")
            self.preprocess_data = pickle.load(_fp)
        else:
            self.preprocess()
    ...
    def setup_detectron2(self):
        ...
        if self.cfg.phalp.detector == 'maskrcnn':
            self.detectron2_cfg = model_zoo.get_config('new_baselines/mask_rcnn_regnety_4gf_dds_FPN_400ep_LSJ.py',
                                                       trained=True)
    ...
    def setup_deepsort(self):
        ...
        metric = nn_matching.NearestNeighborDistanceMetric(self.cfg, self.cfg.phalp.hungarian_th,
                                                           self.cfg.phalp.past_lookback)
        self.tracker = Tracker(self.cfg, metric, max_age=self.cfg.phalp.max_age_track, n_init=self.cfg.phalp.n_init,
    ...
    def preprocess(self):
        ...
        # self.detector.to(device=self.device)
        ...
        pkl_path = self.cfg.video.output_dir + '/results/' + self.cfg.track_dataset + "_" + str(
            self.cfg.video_seq) + '.pkl'
        ...
        list_of_frames = list_of_frames if self.cfg.phalp.start_frame == -1 else list_of_frames[
                                                                                 self.cfg.phalp.start_frame:self.cfg.phalp.end_frame]
        list_of_shots = self.get_list_of_shots(list_of_frames)

        tracked_frames = []
        final_visuals_dic = {}

        for t_, frame_name in progress_bar(enumerate(list_of_frames),
                                           description="Preprocessing : " + self.cfg.video_seq,
                                           total=len(list_of_frames), disable=False):
            ...
            ############ detection ##############
            pred_bbox, pred_masks, pred_scores, pred_classes, gt_tids, gt_annots = self.get_detections(image_frame,
                                                                                                       frame_name, t_,
                                                                                                       additional_data,
                                                                                                       measurments)

            self.preprocess_data[frame_name] = {'pred_bbox': pred_bbox,
        ...
        _fp = open(self.preprocessed_data_file_name, "wb")
        pickle.dump(self.preprocess_data, _fp)
    ...
    def track(self):
        ...
        try:
            ...
            for t_, frame_name in enumerate(list_of_frames):
                ...
                ############ detection ##############
                pred_bbox = self.preprocess_data[frame_name]['pred_bbox']
                pred_masks = self.preprocess_data[frame_name]['pred_masks']
                pred_scores = self.preprocess_data[frame_name]['pred_scores']
                pred_classes = self.preprocess_data[frame_name]['pred_classes']
                gt_tids = self.preprocess_data[frame_name]['gt_tids']
                gt_annots = self.preprocess_data[frame_name]['gt_annots']
                ...
                extra_data = self.run_additional_models(image_frame, pred_bbox, pred_masks, pred_scores, pred_classes,
                                                        frame_name, t_, measurments, gt_tids, gt_annots)

                ############ HMAR ##############
                detections = self.get_human_features(image_frame, pred_masks, pred_bbox, pred_scores, frame_name,
                                                     pred_classes, t_, measurments, gt_tids, gt_annots, extra_data)
                ...
                ############ record the results ##############
                final_visuals_dic.setdefault(frame_name,
                ...
                for tracks_ in self.tracker.tracks:
                    ...
                    for pkey_ in prediction_keys:  final_visuals_dic[frame_name][pkey_].append(
                    ...
                    if (tracks_.time_since_update == 0):
                        ...
                        if (tracks_.hits == self.cfg.phalp.n_init):
                            for pt in range(self.cfg.phalp.n_init - 1):
                                ...
                                for hkey_ in history_keys:    final_visuals_dic[frame_name_][hkey_].append(
                                    track_data_hist_[hkey_])
                                for pkey_ in prediction_keys: final_visuals_dic[frame_name_][pkey_].append(
            ...
            if (self.cfg.use_gt): joblib.dump(self.tracker.tracked_cost, self.cfg.video.output_dir + '/results/' + str(
                self.cfg.video_seq) + '_' + str(self.cfg.phalp.start_frame) + '_distance.pkl')
    ...
    def get_saved_detections(self, image, frame_name, t_, additional_data=None, measurements=None):
    ...
    def run_additional_models(self, image_frame, pred_bbox, pred_masks, pred_scores, pred_classes, frame_name, t_,
                              measurments, gt_tids, gt_annots):
    ...
    def get_human_features(self, image, seg_mask, bbox, score, frame_name, cls_id, t_, measurments, gt=1, ann=None,
        ...
        with torch.no_grad():
            ...
            pred_smpl_params, pred_joints_2d, pred_joints, pred_cam = self.HMAR.get_3d_parameters(hmar_out['pose_smpl'],
                                                                                                  hmar_out['pred_cam'],
                                                                                                  center=(np.array(
        ...
        if (self.cfg.detect_shots):
            if (isinstance(list_of_frames[0], str)):
                ...
                for ft_, fname_ in enumerate(list_of_frames):
                    ...
                    if (ft_ == 0):
                        video_file = cv2.VideoWriter(video_tmp_name, cv2.VideoWriter_fourcc(*'mp4v'), 24,
            ...
            log.info("Detected shot change at frame" + "s" * min(0, len(list_of_shots) - 1) + ": " + ", ".join(
                map(str, list_of_shots)))
